...
type StructuredBackendResult = StructuredBackendSuccess | StructuredBackendFailure;
...
/** Judge rubric handler: structured evaluation with normalized score/result output */
export class JudgeRubricHandler implements Handler {
  ...
  async execute(
    ...
    const inputKey = getAttrString(node, "judge.input_key") || "last_response";
    const artifact = context.get(inputKey);
    if (artifact === undefined || artifact === null || String(artifact).trim() === "") {
      return failOutcome(
        `judge.rubric requires context value at '${inputKey}'`,
      );
    }

    const threshold = parseFractionAttr(node, "judge.threshold", 0.75);
    if (threshold === null) {
      return failOutcome("judge.rubric requires judge.threshold in range [0,1]");
    }

    const prompt = [
      node.prompt || node.label || "Evaluate the artifact against the rubric.",
      graph.attrs.goal ? `Pipeline goal:\n${graph.attrs.goal}` : "",
      getAttrString(node, "judge.criteria")
        ? `Rubric criteria:\n${getAttrString(node, "judge.criteria")}`
        : "",
      `Artifact from context key '${inputKey}':\n${String(artifact)}`,
    ...
    const score = parseStructuredScore(
      backendResult.data.score ?? backendResult.data.overall_score,
    );
    const summary = parseNonEmptyString(
      backendResult.data.summary ?? backendResult.data.rationale,
    );
    ...
    const result = score >= threshold ? "pass" : "revise";
    const outcome = {
      status: result === "pass" ? StageStatus.SUCCESS : StageStatus.PARTIAL_SUCCESS,
      contextUpdates: {
        "judge.rubric.score": score,
        "judge.rubric.summary": summary,
        "judge.rubric.result": result,
      },
      notes: `judge.rubric ${result}: score=${score.toFixed(3)} threshold=${threshold.toFixed(3)}`,
    } satisfies Outcome;
    writeStatus(backendResult.stageDir, outcome);
    return outcome;
  }
...
/** Failure analysis handler: structured classification for downstream routing */
export class FailureAnalyzeHandler implements Handler {
  ...
  async execute(
    ...
    const inputKey = getAttrString(node, "failure.input_key") || "failure.reason";
    const failureInput = context.get(inputKey);
    if (
      ...
      return failOutcome(
        `failure.analyze requires context value at '${inputKey}'`,
      );
    }
    ...
    const prompt = [
      ...
      graph.attrs.goal ? `Pipeline goal:\n${graph.attrs.goal}` : "",
      getAttrString(node, "failure.hints")
        ? `Analysis hints:\n${getAttrString(node, "failure.hints")}`
        : "",
      `Failure context from '${inputKey}':\n${String(failureInput)}`,
      `Return JSON with fields class (${FAILURE_CLASSES.join("|")}), summary (string), recommendation (string).`,
    ].filter(Boolean).join("\n\n");
    ...
    const failureClass = normalizeFailureClass(
      backendResult.data.class ?? backendResult.data.failure_class,
    );
    const summary = parseNonEmptyString(backendResult.data.summary);
    const recommendation = parseNonEmptyString(
      backendResult.data.recommendation,
    );
    ...
    const outcome = successOutcome({
      contextUpdates: {
        "failure.analyze.class": failureClass,
        "failure.analyze.summary": summary,
        "failure.analyze.recommendation": recommendation,
      },
...
/** Confidence gate: deterministic decision from normalized quality/failure signals */
export class ConfidenceGateHandler implements Handler {
  async execute(
    ...
    if (threshold === null) {
      return failOutcome("confidence.gate requires confidence.threshold in range [0,1]");
    }

    const scoreKey = getAttrString(node, "confidence.score_key") || "judge.rubric.score";
    const failureClassKey =
      getAttrString(node, "confidence.failure_class_key") || "failure.analyze.class";
    const failureClass = normalizeFailureClass(context.get(failureClassKey));
    const score = resolveConfidenceScore(context, scoreKey, failureClass);
    if (score === null) {
      return failOutcome(
        `confidence.gate requires a normalized score at '${scoreKey}' or prior quality/failure signals`,
      );
    }

    const escalateClasses = parseCsvAttr(
      node,
      "confidence.escalate_classes",
      ["quality_gap", "tool_error", "spec_mismatch"],
    );

    let decision: "autonomous" | "escalate" = score >= threshold
      ? "autonomous"
      : "escalate";
    let reason = score >= threshold
      ? `score ${score.toFixed(3)} met threshold ${threshold.toFixed(3)}`
      : `score ${score.toFixed(3)} fell below threshold ${threshold.toFixed(3)}`;

    if (failureClass && escalateClasses.includes(failureClass)) {
      decision = "escalate";
    ...
    return {
      ...
      contextUpdates: {
        "confidence.gate.decision": decision,
        "confidence.gate.score": score,
        "confidence.gate.reason": reason,
      },
...
/** Quality gate: deterministic condition checks over the current context */
export class QualityGateHandler implements Handler {
  async execute(
    ...
    const checks = parseQualityChecks(node);
    if (!checks) {
      return failOutcome(
        "quality.gate requires quality.checks as a JSON array of checks",
      );
    }
    ...
    return {
      ...
      contextUpdates: {
        "quality.gate.result": passed ? "pass" : "fail",
        "quality.gate.failed_checks": JSON.stringify(failedChecks),
        "quality.gate.summary": passed
          ? `All ${checks.length} quality checks passed`
          : `Failed checks: ${failedChecks.join(", ")}`,
      },
      notes: `quality.gate ${passed ? "pass" : "fail"} (${checks.length} checks)`,
    };
  }
}
...
function parseStructuredScore(value: unknown): number | null {
...
function normalizeFailureClass(value: unknown): FailureClass | "" {
...
function parseQualityChecks(node: GraphNode): QualityGateCheck[] | null {
  ...
  try {
    ...
    const checks = parsed.map((entry, index) => {
      if (typeof entry === "string" && entry.trim()) {
        return {
          label: `check_${index + 1}`,
          condition: entry,
          summary: entry,
        } satisfies QualityGateCheck;
      }
      if (!entry || typeof entry !== "object") {
        return null;
      }
      const label = parseNonEmptyString((entry as Record<string, unknown>).label)
        || `check_${index + 1}`;
      const condition = parseNonEmptyString(
      ...
      return { label, condition, summary } satisfies QualityGateCheck;
    });
    return checks.every(Boolean) ? (checks as QualityGateCheck[]) : null;
  } catch {
    return null;
  }
}
...
function resolveConfidenceScore(
  ...
  const qualityGateResult = context.getString("quality.gate.result");
...
async function executeStructuredBackend(
  node: GraphNode,
  context: Context,
  graph: Graph,
  logsRoot: string,
