...
- прогнан пакетный набор тестов: `pnpm --filter @attractor/core test` -> `384/384` тестов проходят.
...
| `4.10 Tool Handler` и spec `9.7 Tool Call Hooks` | `Partial` | `packages/attractor-core/src/handlers/handlers.ts:635`, `packages/attractor-core/tests/tool-hooks.test.ts` | `pre_hook`/`post_hook` работают вокруг `tool_command` у tool-node. Но spec `9.7` говорит про `tool_hooks.pre/post` вокруг каждого LLM tool call, а не только shell tool-node. Эта часть не закрыта end-to-end. |
| `5. State and Context` — context store, artifacts, checkpoint, resume | `Implemented` | `packages/attractor-core/src/state/context.ts`, `packages/attractor-core/src/state/checkpoint.ts`, `packages/attractor-core/src/engine/runner.ts:262`, `packages/attractor-cli/src/index.ts:82`, `packages/attractor-core/tests/state.test.ts`, `packages/attractor-core/tests/integration.test.ts`, `packages/attractor-core/tests/fidelity.test.ts` | Контекст, artifacts, checkpoint/resume и resume-fidelity semantics реализованы. CLI тоже экспонирует resume flow через `--resume-from`, а server умеет прокинуть `resumeFrom` в runner. |
...
| `9.6 Observability and Events` | `Partial` | `packages/attractor-core/src/events/index.ts:1`, `packages/attractor-core/src/engine/runner.ts:232` | Типы событий объявлены почти полностью, но в рантайме эмитится только часть: pipeline/stage/checkpoint/retry/loop restart. `parallel_*`, `interview_*` и `stage_failed` объявлены, но по коду не эмитятся. |
